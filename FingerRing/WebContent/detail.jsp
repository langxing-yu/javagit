...
%>
  ...
  <form id="aspnetForm" action="/jewelry/404.jsp" method="post" name="aspnetForm"> 
   <div> 
...
//    });
    ...
    <div class="cort"> 
     ...
     <%
			// 获取上个页面传递过来的gid
			String gidst = request.getParameter("gid");
			if(null==gidst || gidst.equals("")){
				out.println("<script>location.href='index.jsp';</script>");
				return;
			}
			int goid = Integer.parseInt(gidst);
			// 调用service层方法，获取goods对象
			GoodsService bs = new GoodsService();
			Goods goods = bs.findGoodsById(gid);
			
			// 在session中存入当前页面路径
			session.setAttribute("page", "detail.jsp?gid=" + goid);
		%>
     <div class="cort"> 
      <div class="tobuy cmain"> 
       <div class="zbk_top spalid"> 
        <span>您当前的位置：</span> 
        <span id="ctl00_content_website_SiteMapPath1"><a href="#ctl00_content_website_SiteMapPath1_SkipLink"></a><span> <a target="_blank" href="index.jsp">Darry Ring</a> </span><span> <em>&gt;</em> </span><span> <a target="_blank" href="lists.jsp">珠宝饰品</a> </span><span> <em>&gt;</em> </span><span> <a target="_blank" href="/jewelry_4">吊坠</a> </span><span> <em>&gt;</em> </span><span> <span><%=goods.getGname() %></span> </span><a id="ctl00_content_website_SiteMapPath1_SkipLink"></a></span> 
       </div> 
       <!--购买页中间--> 
       <div class="buy_cort"> 
        <script type="text/javascript">
    ...
    $(function () {
        ...
        $(".share_sc").click(function () {
            if ('false' == 'false') {
                alert("请登录！");
                window.location.href = 'login.jsp?forward=index.jsp/jewelry/404.jsp';
            }
        });
        ...
        <div class="buycort_center fl"> 
         <ul class="ul_center"> 
          <li id="magnifier2083" style="display: list-item;"> <img src="<%=good.getSrc() %>" alt="锁住一生 LOCK套链 0.006 G-I" /> <span style="position: absolute; left: 248px; top: 248px; display: none; width: 150px; height: 150px; background: rgb(153, 153, 153) none repeat scroll 0% 0%; border: 1px solid rgb(0, 0, 0); cursor: move; opacity: 0.4;"></span>
        ...
        <div class="buycort_right fr"> 
         ...
         <div class="byright_top"> 
          <p><%=goods.getGname() %><%=goods.getNum() %></p> 
          <p> <span><%=goods.getRate() %></span> </p> 
          <div class="byright_top-xq"> 
           <i>最近售出：<%=goods.getSales() %></i> 
           <i>用户评论：6</i> 
           <i>收藏人气：25</i> 
          </div> 
         </div> 
         ...
         <div class="pho_cs" id="ctl00_content_zbDiv"> 
          <p> <span>尺寸：</span> <i><%=goods.getSize() %></i> </p> 
...
//        });
 -->
  	...
  	<script>
            function toPay() {
               location.href="pay.jsp?gid=<%=goods.getGid() %>";
            }
    </script> 
         ...
         <div id="addCa" class="button buy_button"> 
          <div title="购买darry ring钻戒" class="bt1">
           <span onclick="location.href='upshop?gid=<%= goods.getGid()%>'">加入购物车</span>
          </div> 
          <div title="把darry ring加入购物车" class="bt2">
           <span onClick="toPay()">立即购买</span>
          </div> 
         </div> 
        </div> 
        <!--购买页中间的右边end--> 
          ...
          <table cellspacing="0" cellpadding="0" border="0" class="border_cs-table"> 
           <tbody>
            <tr> 
             ...
             <td class="border_cs-td2"> <span>产品编号：</span> <em><%=goods.getNum() %></em> </td> 
            ...
            <tr> 
             ...
             <td class="border_cs-td5"> <span>颜 色：</span> <em><%=goods.getColour() %></em> </td> 
    ...
    function CommentToNext() {
        ...
        <div class="ask cmain"> 
         ...
         <div class="talk_it"> 
          ...
          <div class="talkit_top"> 
           <div class="bt2 fl">
            <a target="_blank" href="/member/myevaluate.jsp">我要评论</a>
           </div> 
          ...
          <div id="questionContent"> 
           ...
           <div class="every_talk"> 
            ...
            <div class="question_left fl"> 
             ...
             <p class="answer"> <i></i> <span> <span style="margin-top:1px;">A：您好，DR支持您收到货品后15天内无理由退换货的哦。你收到商品如果不是十分满意，可以联系客服选择更换款式或是退货处理的。但因DR都是定制商品，如非质量原因申请退货，则需要扣除一定的定制费用。 注：扣费标准请参考index.jsp/help_se/84.jsp</span> </span> </p> 
            </div> 
            <!--每一条评论左边end--> 
            <!--每一条评论右边--> 
            <div class="question_right fr"> 
      ...
      <div class="yz_password"> 
       <div class="yzp_border"> 
        ...
        <div style="display: none" class="yzpb_cort toyz_buyit wdl"> 
         ...
         <p> 您尚未登录，无法购买请<a href="javascript:backLogin();">登录</a>/<a href="reg.jsp">注册</a></p> 
        </div> 
        <!--未登陆 end--> 
        <!--购买过Darry钻戒--> 
    ...
    function backLogin() {
        ...
        window.location.href = "login.jsp?forward=" + currentUrl;
    }
